...
def _infer_train_yaml(command: list[str]) -> str | None:
...
def generate_plots(exp_dir: Path | str) -> None:
    ...
    try:
        ...
        from monitor_common.sft_plots import plot_sft_training_summary

        exp_summary = _load_json(exp_dir / "experiment_summary.json")
        sft_args = _load_json(exp_dir / "sft_args.json")
        command = sft_args.get("command", [])
        generate_sft_metrics_summary(
            exp_dir=exp_dir,
            output_dir=exp_summary.get("output_dir"),
            train_yaml=_infer_train_yaml(command),
            workdir=sft_args.get("workdir"),
        )
        plot_sft_training_summary(exp_dir, out_dir)
    except Exception as exc:
        print(f"[plot] extended SFT metrics skipped: {exc}")
