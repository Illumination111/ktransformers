...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pretrain-metrics-json", default=None, help="optional JSON with task metrics before SFT")
    parser.add_argument("--finetuned-metrics-json", default=None, help="optional JSON with task metrics after SFT")
...
def write_summary(
    ...
    monitor_metrics = {}
    try:
        from monitor_common.sft_metrics import generate_sft_metrics_summary

        monitor_metrics = generate_sft_metrics_summary(
            exp_dir=exp_dir,
            output_dir=output_dir,
            train_yaml=train_yaml,
            workdir=args.workdir,
            before_metrics_path=args.pretrain_metrics_json,
            after_metrics_path=args.finetuned_metrics_json,
        )
    except Exception as exc:
        monitor_metrics = {"error": str(exc)}
    ...
    summary = {
        ...
        "monitor_metrics_summary": monitor_metrics,
    }
...
def main() -> None:
    ...
    train_yaml = infer_train_yaml(cmd)
    output_dir = infer_output_dir(train_yaml, args.workdir)
    ...
    if args.dry_run:
        ...
        write_summary(exp_dir, time.monotonic(), 0, output_dir, train_yaml, args)
        return
