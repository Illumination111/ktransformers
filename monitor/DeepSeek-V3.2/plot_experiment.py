...
def plot_memory_timeline(records, startup, exp_dir: Path, out_dir: Path):
    ...
    # 纵轴上限：峰值内存向上取整到下一个百位（如 160→200，210→300），
    # 避免小规模模型的占用曲线被压缩在图像底部
    peak_used = max(cpu_used) if cpu_used else 0
    y_max = (int(peak_used) // 100 + 1) * 100 if peak_used > 0 else 100
    if cpu_total:
        y_max = min(y_max, cpu_total)  # 不超过物理总内存
    ax_cpu.set_ylim(0, y_max)

    if cpu_total:
        ax_cpu.plot([], [], " ", label=f"Total: {cpu_total:.0f} GB")
        if cpu_total <= y_max:
            ax_cpu.axhline(cpu_total, color="red", lw=1, ls="--", alpha=0.5)
